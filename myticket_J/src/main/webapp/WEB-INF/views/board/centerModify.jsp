		...
		<div id="page-content-wrapper">
				...
				<form name="form" id="form" role="form" method="post">
					<div class="mb-3">
						...
						<input type="text" class="form-control" name="title" id="title" value="${center.title}" maxlength="100" >
					...
					<div class="mb-3">
						...
						<select name="saleCode" class="form-select">
							<option value="${center.saleCode}" selected="selected">현재 상품(${center.saleCode})</option>	
							<c:forEach var="codeList" items="${saleCode}">
							  	<option value="${codeList.saleCode}">${codeList.name}(${codeList.saleCode})</option>	
							</c:forEach>
						</select>
					</div>
					<div class="mb-3">
						<label for="id">작성자</label>
						<input type="text" class="form-control" name="id" id="id" value="${center.id}" readonly="readonly">
					</div>
					<div class="mb-3">
						<label for="content">내용</label>
						<textarea class="form-control" rows="5" name="content" id="content" maxlength="1000" >${center.content}</textarea>
						<div class ="invalid-feedback" style="display:none;">
					      	내용을 입력하세요
					    </div>
					</div>
					...
					<div class="d-grid gap-2 d-md-flex justify-content-md-end">
						<button type="submit" class="btn btn-sm btn-primary" id="btnModify" formaction="${root }CenterModifyProc?reUri=${reUri}">수정</button>
