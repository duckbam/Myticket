...
<c:if test="${empty bookView.bookName || empty bookView.bookPhone || empty bookView.bookCount}">
	<script>
		alert('필수 항목 입력 후 이용하실 수 있습니다.')
		history.back();
	</script>
</c:if>
...
<script>
	$("document").ready(function(){
		$("div.bookData").css("cursor", "pointer").click(function(){
			let bookData = $(this).attr("id");
			$("#bookData").val(bookData);
			$("#f").attr("action", "${root}bookInsertProc");
			$("#f").submit();
		})
	});
</script>
					...
					<input type="hidden" name="bookData" id="bookData">
						<table class="table">
							...
							<col width="65%">
								...
								<tr>
								    <th colspan="2" style="text-align: center; width: 500px;"><h2><b>예약 완료 / 구매하기</b></h2></th>
							   	</tr>
								...
								<tr>
									...
									<td style="text-align: left;">${book.saleCode}</td>
								</tr>
								<tr>
							   	...
							   	<tr>
								    ...
								    <td style="text-align: left;">${bookView.bookName}</td> 
							   	</tr>
							   	<tr>
								    <td style="text-align: center;">구매자 핸드폰번호</td>
								    <td style="text-align: left;">${bookView.bookPhone}</td>
							   	</tr>
								<tr>
									<td style="text-align: center;">선택한 공연 날짜</td>
									<td style="text-align: left;">${bookView.bookDate}</td>
								</tr>
								<tr>
									<td style="text-align: center;">관람 나이 </td>
									<c:choose>
										<c:when test="${book.age == 0}">
											<td style="text-align: left;">전체 관람</td>
										</c:when>
										<c:otherwise>
											<td style="text-align: left;">${book.age}세 관람</td>
										</c:otherwise>
									</c:choose>
								</tr>
								<tr>
									<td style="text-align: center;">잔여 좌석 </td>
									<td style="text-align: left;">${book.seat}</td>
								</tr>
								<tr>
									<td style="text-align: center;">가격 </td>
									<td style="text-align: left;"><fmt:formatNumber type="number" value="${book.price}"/>원</td>
								</tr>
								<tr>
									<td style="text-align: center;">구매 매수 </td>
									<td style="text-align: left;">${bookView.bookCount}</td>
								</tr>
								<tr>
									<td style="text-align: center;">가격 </td>
									<td style="text-align: left;"><fmt:formatNumber type="number" value="${book.price * bookView.bookCount}"/>원</td>
								</tr>
								
								<tr>
									<td style="height: 200px;" colspan="2"><h4><b>&nbsp;구매 하기 안내</b></h4><br>&nbsp;예약 내용을 확인해주세요.<br><br>&nbsp;예약 취소는 ${bookView.bookDate}까지 가능합니다. <br><br>&nbsp;취소를 원하시면 마이페이지에서 이용해주세요. <br><br>&nbsp;문의를 원하실때에는 로그인 후 아래의 문의하기로 이동해주세요.
									<br><br><br><a href="${root}centerProc" style="text-decoration: none; color: black;"><h4>문의하기</h4></a></td>
								</tr>
						</table>
							<div id="${book.saleCode}" class="bookData" style="padding:10px; width: 150px; height: 180px; display: inline; background-color: #D5D5D5; border: none; border-radius: 3px; font-weight: bold;">구매 하기
							</div>
							&nbsp;&nbsp;
